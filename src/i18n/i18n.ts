    export const translations = {
        ru: {
            // Основное
            appName: 'FinFlow',
            tagline: 'Умное управление финансами',

            // Действия
            addTransaction: 'Добавить транзакцию',
            newTransaction: 'Новая транзакция',
            save: 'Сохранить',
            cancel: 'Отмена',
            delete: 'Удалить',
            reset: 'Сбросить',

            // Финансы
            balance: 'Баланс',
            income: 'Доходы',
            expense: 'Расходы',
            transactions: 'Транзакции',

            // Форма
            amount: 'Сумма',
            category: 'Категория',
            description: 'Описание',
            date: 'Дата',
            type: 'Тип',

            // Фильтры
            filters: 'Фильтры',
            allTypes: 'Все типы',
            allCategories: 'Все категории',
            from: 'От',
            to: 'До',

            // Сообщения
            noTransactions: 'Транзакций пока нет',
            addFirst: 'Добавьте первую транзакцию',
            confirmDelete: 'Удалить транзакцию?',
            noData: 'Нет данных',

            // Валидация
            required: 'обязательно',
            amountRequired: 'Укажите сумму',
            categoryRequired: 'Выберите категорию',
            dateRequired: 'Укажите дату',
            invalidAmount: 'Введите корректную сумму',
            amountMustBePositive: 'Сумма должна быть больше 0',
            descriptionPlaceholder: 'Например: Покупка продуктов',

            // Навигация
            dashboard: 'Главная',
            analytics: 'Аналитика',
            analyticsDescription: 'Визуализация и анализ ваших финансов',

            // Аналитика
            week: 'Неделя',
            month: 'Месяц',
            year: 'Год',
            totalIncome: 'Всего доходов',
            totalExpenses: 'Всего расходов',
            savingsRate: 'Процент экономии',
            avgDailyExpense: 'Средний расход в день',
            topCategory: 'Главная категория',
            noExpenseData: 'Нет данных о расходах',
            noTransactionData: 'Нет данных о транзакциях',
            expensesByCategory: 'Расходы по категориям',
            incomeExpenseTrend: 'Тренд доходов и расходов',

            // НОВЫЕ ПЕРЕВОДЫ ДЛЯ АНАЛИТИКИ
            financialAnalytics: 'Финансовая аналитика',
            selectPeriod: 'Выберите период',
            lastWeek: 'За последнюю неделю',
            lastMonth: 'За последний месяц',
            lastYear: 'За последний год',
            days: 'дней',
            netBalance: 'Чистый баланс',
            allIncomingTransactions: 'Все входящие транзакции',
            allExpenseTransactions: 'Все расходные транзакции',
            positiveBalance: 'Положительный баланс',
            negativeBalance: 'Отрицательный баланс',
            ofIncomeSaved: 'от дохода сохранено',
            topSpendingCategory: 'Топ категория расходов',
            highestExpenseCategory: 'Наибольшая категория расходов',
            transactionCount: 'Количество транзакций',
            totalTransactionsInPeriod: 'Всего транзакций за период',
            averageTransaction: 'Средняя транзакция',
            perTransaction: 'за транзакцию',
            expenseDistribution: 'Распределение расходов',
            incomeVsExpenses: 'Доходы vs Расходы',

            // Графики
            topCategories: 'Топ категорий',
            balanceTrend: 'Динамика баланса',
            incomeVsExpense: 'Доходы vs Расходы',
            totalTransactions: 'Всего транзакций',

            // Категории - Расходы
            'category.food': 'Еда и продукты',
            'category.transport': 'Транспорт',
            'category.housing': 'Жильё',
            'category.entertainment': 'Развлечения',
            'category.health': 'Здоровье',
            'category.education': 'Образование',
            'category.clothing': 'Одежда',
            'category.subscriptions': 'Подписки',
            'category.other': 'Другое',

            // Категории - Доходы
            'category.salary': 'Зарплата',
            'category.freelance': 'Фриланс',
            'category.investment': 'Инвестиции',
            'category.gift': 'Подарки',

            // Настройки
            'settings.language': 'Язык',
            'settings.currency': 'Валюта',
            'settings.theme': 'Тема',
        },

        en: {
            appName: 'FinFlow',
            tagline: 'Smart Finance Management',
            addTransaction: 'Add Transaction',
            newTransaction: 'New Transaction',
            save: 'Save',
            cancel: 'Cancel',
            delete: 'Delete',
            reset: 'Reset',
            balance: 'Balance',
            income: 'Income',
            expense: 'Expenses',
            transactions: 'Transactions',
            amount: 'Amount',
            category: 'Category',
            description: 'Description',
            date: 'Date',
            type: 'Type',
            filters: 'Filters',
            allTypes: 'All Types',
            allCategories: 'All Categories',
            from: 'From',
            to: 'To',
            noTransactions: 'No transactions yet',
            addFirst: 'Add your first transaction',
            confirmDelete: 'Delete transaction?',
            noData: 'No data',
            required: 'required',
            amountRequired: 'Enter amount',
            categoryRequired: 'Select category',
            dateRequired: 'Select date',
            invalidAmount: 'Enter valid amount',
            amountMustBePositive: 'Amount must be positive',
            descriptionPlaceholder: 'E.g.: Grocery shopping',
            dashboard: 'Dashboard',
            analytics: 'Analytics',
            analyticsDescription: 'Visualization and analysis of your finances',
            week: 'Week',
            month: 'Month',
            year: 'Year',
            totalIncome: 'Total Income',
            totalExpenses: 'Total Expenses',
            savingsRate: 'Savings Rate',
            avgDailyExpense: 'Average Daily Expense',
            topCategory: 'Top Category',
            noExpenseData: 'No expense data',
            noTransactionData: 'No transaction data',
            expensesByCategory: 'Expenses by Category',
            incomeExpenseTrend: 'Income & Expense Trend',

            // NEW ANALYTICS TRANSLATIONS
            financialAnalytics: 'Financial Analytics',
            selectPeriod: 'Select Period',
            lastWeek: 'Last Week',
            lastMonth: 'Last Month',
            lastYear: 'Last Year',
            days: 'days',
            netBalance: 'Net Balance',
            allIncomingTransactions: 'All incoming transactions',
            allExpenseTransactions: 'All expense transactions',
            positiveBalance: 'Positive balance',
            negativeBalance: 'Negative balance',
            ofIncomeSaved: 'of income saved',
            topSpendingCategory: 'Top Spending Category',
            highestExpenseCategory: 'Highest expense category',
            transactionCount: 'Transaction Count',
            totalTransactionsInPeriod: 'Total transactions in period',
            averageTransaction: 'Average Transaction',
            perTransaction: 'per transaction',
            expenseDistribution: 'Expense Distribution',
            incomeVsExpenses: 'Income vs Expenses',

            topCategories: 'Top Categories',
            balanceTrend: 'Balance Trend',
            incomeVsExpense: 'Income vs Expenses',
            totalTransactions: 'Total Transactions',
            'category.food': 'Food & Groceries',
            'category.transport': 'Transport',
            'category.housing': 'Housing',
            'category.entertainment': 'Entertainment',
            'category.health': 'Health',
            'category.education': 'Education',
            'category.clothing': 'Clothing',
            'category.subscriptions': 'Subscriptions',
            'category.other': 'Other',
            'category.salary': 'Salary',
            'category.freelance': 'Freelance',
            'category.investment': 'Investment',
            'category.gift': 'Gifts',
            'settings.language': 'Language',
            'settings.currency': 'Currency',
            'settings.theme': 'Theme',
        },

        fr: {
            appName: 'FinFlow',
            tagline: 'Gestion intelligente des finances',
            addTransaction: 'Ajouter une transaction',
            newTransaction: 'Nouvelle transaction',
            save: 'Enregistrer',
            cancel: 'Annuler',
            delete: 'Supprimer',
            reset: 'Réinitialiser',
            balance: 'Solde',
            income: 'Revenus',
            expense: 'Dépenses',
            transactions: 'Transactions',
            amount: 'Montant',
            category: 'Catégorie',
            description: 'Description',
            date: 'Date',
            type: 'Type',
            filters: 'Filtres',
            allTypes: 'Tous les types',
            allCategories: 'Toutes les catégories',
            from: 'De',
            to: 'À',
            noTransactions: 'Aucune transaction',
            addFirst: 'Ajoutez votre première transaction',
            confirmDelete: 'Supprimer la transaction?',
            noData: 'Aucune donnée',
            required: 'obligatoire',
            amountRequired: 'Entrez le montant',
            categoryRequired: 'Sélectionnez une catégorie',
            dateRequired: 'Sélectionnez une date',
            invalidAmount: 'Entrez un montant valide',
            amountMustBePositive: 'Le montant doit être positif',
            descriptionPlaceholder: 'Ex: Courses alimentaires',
            dashboard: 'Tableau de bord',
            analytics: 'Analytique',
            analyticsDescription: 'Visualisation et analyse de vos finances',
            week: 'Semaine',
            month: 'Mois',
            year: 'Année',
            totalIncome: 'Revenus totaux',
            totalExpenses: 'Dépenses totales',
            savingsRate: "Taux d'épargne", // Исправлено: экранирование апострофа
            avgDailyExpense: 'Dépense quotidienne moyenne',
            topCategory: 'Catégorie principale',
            noExpenseData: 'Aucune donnée de dépense',
            noTransactionData: 'Aucune donnée de transaction',
            expensesByCategory: 'Dépenses par catégorie',
            incomeExpenseTrend: 'Tendance revenus/dépenses',

            // NEW ANALYTICS TRANSLATIONS
            financialAnalytics: 'Analyse Financière',
            selectPeriod: 'Sélectionner la période',
            lastWeek: 'La semaine dernière',
            lastMonth: 'Le mois dernier',
            lastYear: "L'année dernière",
            days: 'jours',
            netBalance: 'Solde net',
            allIncomingTransactions: 'Toutes les transactions entrantes',
            allExpenseTransactions: 'Toutes les transactions de dépenses',
            positiveBalance: 'Solde positif',
            negativeBalance: 'Solde négatif',
            ofIncomeSaved: 'du revenu économisé',
            topSpendingCategory: 'Catégorie de dépenses principale',
            highestExpenseCategory: 'Catégorie de dépenses la plus élevée',
            transactionCount: 'Nombre de transactions',
            totalTransactionsInPeriod: 'Total des transactions sur la période',
            averageTransaction: 'Transaction moyenne',
            perTransaction: 'par transaction',
            expenseDistribution: 'Répartition des dépenses',
            incomeVsExpenses: 'Revenus vs Dépenses',

            topCategories: 'Top catégories',
            balanceTrend: 'Tendance du solde',
            incomeVsExpense: 'Revenus vs Dépenses',
            totalTransactions: 'Total des transactions',
            'category.food': 'Alimentation',
            'category.transport': 'Transport',
            'category.housing': 'Logement',
            'category.entertainment': 'Loisirs',
            'category.health': 'Santé',
            'category.education': 'Éducation',
            'category.clothing': 'Vêtements',
            'category.subscriptions': 'Abonnements',
            'category.other': 'Autre',
            'category.salary': 'Salaire',
            'category.freelance': 'Freelance',
            'category.investment': 'Investissement',
            'category.gift': 'Cadeaux',
            'settings.language': 'Langue',
            'settings.currency': 'Devise',
            'settings.theme': 'Thème',
        },

        de: {
            appName: 'FinFlow',
            tagline: 'Intelligente Finanzverwaltung',
            addTransaction: 'Transaktion hinzufügen',
            newTransaction: 'Neue Transaktion',
            save: 'Speichern',
            cancel: 'Abbrechen',
            delete: 'Löschen',
            reset: 'Zurücksetzen',
            balance: 'Saldo',
            income: 'Einkommen',
            expense: 'Ausgaben',
            transactions: 'Transaktionen',
            amount: 'Betrag',
            category: 'Kategorie',
            description: 'Beschreibung',
            date: 'Datum',
            type: 'Typ',
            filters: 'Filter',
            allTypes: 'Alle Typen',
            allCategories: 'Alle Kategorien',
            from: 'Von',
            to: 'Bis',
            noTransactions: 'Noch keine Transaktionen',
            addFirst: 'Erste Transaktion hinzufügen',
            confirmDelete: 'Transaktion löschen?',
            noData: 'Keine Daten',
            required: 'erforderlich',
            amountRequired: 'Betrag eingeben',
            categoryRequired: 'Kategorie auswählen',
            dateRequired: 'Datum auswählen',
            invalidAmount: 'Gültigen Betrag eingeben',
            amountMustBePositive: 'Betrag muss positiv sein',
            descriptionPlaceholder: 'Z.B.: Lebensmitteleinkauf',
            dashboard: 'Dashboard',
            analytics: 'Analytik',
            analyticsDescription: 'Visualisierung und Analyse Ihrer Finanzen',
            week: 'Woche',
            month: 'Monat',
            year: 'Jahr',
            totalIncome: 'Gesamteinnahmen',
            totalExpenses: 'Gesamtausgaben',
            savingsRate: 'Sparrate',
            avgDailyExpense: 'Durchschnittliche Tagesausgabe',
            topCategory: 'Hauptkategorie',
            noExpenseData: 'Keine Ausgabendaten',
            noTransactionData: 'Keine Transaktionsdaten',
            expensesByCategory: 'Ausgaben nach Kategorie',
            incomeExpenseTrend: 'Einkommen-/Ausgabentrend',

            // NEW ANALYTICS TRANSLATIONS
            financialAnalytics: 'Finanzanalyse',
            selectPeriod: 'Zeitraum auswählen',
            lastWeek: 'Letzte Woche',
            lastMonth: 'Letzten Monat',
            lastYear: 'Letztes Jahr',
            days: 'Tage',
            netBalance: 'Netto-Saldo',
            allIncomingTransactions: 'Alle eingehenden Transaktionen',
            allExpenseTransactions: 'Alle Ausgabentransaktionen',
            positiveBalance: 'Positiver Saldo',
            negativeBalance: 'Negativer Saldo',
            ofIncomeSaved: 'des Einkommens gespart',
            topSpendingCategory: 'Top-Ausgabenkategorie',
            highestExpenseCategory: 'Höchste Ausgabenkategorie',
            transactionCount: 'Anzahl der Transaktionen',
            totalTransactionsInPeriod: 'Transaktionen insgesamt im Zeitraum',
            averageTransaction: 'Durchschnittliche Transaktion',
            perTransaction: 'pro Transaktion',
            expenseDistribution: 'Ausgabenverteilung',
            incomeVsExpenses: 'Einkommen vs Ausgaben',

            topCategories: 'Top Kategorien',
            balanceTrend: 'Saldo-Trend',
            incomeVsExpense: 'Einkommen vs Ausgaben',
            totalTransactions: 'Gesamttransaktionen',
            'category.food': 'Essen & Lebensmittel',
            'category.transport': 'Transport',
            'category.housing': 'Wohnen',
            'category.entertainment': 'Unterhaltung',
            'category.health': 'Gesundheit',
            'category.education': 'Bildung',
            'category.clothing': 'Kleidung',
            'category.subscriptions': 'Abonnements',
            'category.other': 'Sonstiges',
            'category.salary': 'Gehalt',
            'category.freelance': 'Freiberuflich',
            'category.investment': 'Investition',
            'category.gift': 'Geschenke',
            'settings.language': 'Sprache',
            'settings.currency': 'Währung',
            'settings.theme': 'Theme',
        },

        es: {
            appName: 'FinFlow',
            tagline: 'Gestión inteligente de finanzas',
            addTransaction: 'Añadir transacción',
            newTransaction: 'Nueva transacción',
            save: 'Guardar',
            cancel: 'Cancelar',
            delete: 'Eliminar',
            reset: 'Restablecer',
            balance: 'Balance',
            income: 'Ingresos',
            expense: 'Gastos',
            transactions: 'Transacciones',
            amount: 'Cantidad',
            category: 'Categoría',
            description: 'Descripción',
            date: 'Fecha',
            type: 'Tipo',
            filters: 'Filtros',
            allTypes: 'Todos los tipos',
            allCategories: 'Todas las categorías',
            from: 'Desde',
            to: 'Hasta',
            noTransactions: 'No hay transacciones',
            addFirst: 'Añade tu primera transacción',
            confirmDelete: '¿Eliminar transacción?',
            noData: 'Sin datos',
            required: 'obligatorio',
            amountRequired: 'Ingrese cantidad',
            categoryRequired: 'Seleccione categoría',
            dateRequired: 'Seleccione fecha',
            invalidAmount: 'Ingrese cantidad válida',
            amountMustBePositive: 'La cantidad debe ser positiva',
            descriptionPlaceholder: 'Ej: Compra de comestibles',
            dashboard: 'Panel',
            analytics: 'Analítica',
            analyticsDescription: 'Visualización y análisis de tus finanzas',
            week: 'Semana',
            month: 'Mes',
            year: 'Año',
            totalIncome: 'Ingresos totales',
            totalExpenses: 'Gastos totales',
            savingsRate: 'Tasa de ahorro',
            avgDailyExpense: 'Gasto diario promedio',
            topCategory: 'Categoría principal',
            noExpenseData: 'Sin datos de gastos',
            noTransactionData: 'Sin datos de transacciones',
            expensesByCategory: 'Gastos por categoría',
            incomeExpenseTrend: 'Tendencia ingresos/gastos',

            // NEW ANALYTICS TRANSLATIONS
            financialAnalytics: 'Análisis Financiero',
            selectPeriod: 'Seleccionar período',
            lastWeek: 'La semana pasada',
            lastMonth: 'El mes pasado',
            lastYear: 'El año pasado',
            days: 'días',
            netBalance: 'Balance neto',
            allIncomingTransactions: 'Todas las transacciones entrantes',
            allExpenseTransactions: 'Todas las transacciones de gastos',
            positiveBalance: 'Balance positivo',
            negativeBalance: 'Balance negativo',
            ofIncomeSaved: 'del ingreso ahorrado',
            topSpendingCategory: 'Categoría de gasto principal',
            highestExpenseCategory: 'Categoría de gasto más alta',
            transactionCount: 'Cantidad de transacciones',
            totalTransactionsInPeriod: 'Total de transacciones en el período',
            averageTransaction: 'Transacción promedio',
            perTransaction: 'por transacción',
            expenseDistribution: 'Distribución de gastos',
            incomeVsExpenses: 'Ingresos vs Gastos',

            topCategories: 'Top categorías',
            balanceTrend: 'Tendencia del balance',
            incomeVsExpense: 'Ingresos vs Gastos',
            totalTransactions: 'Total de transacciones',
            'category.food': 'Comida y comestibles',
            'category.transport': 'Transporte',
            'category.housing': 'Vivienda',
            'category.entertainment': 'Entretenimiento',
            'category.health': 'Salud',
            'category.education': 'Educación',
            'category.clothing': 'Ropa',
            'category.subscriptions': 'Suscripciones',
            'category.other': 'Otro',
            'category.salary': 'Salario',
            'category.freelance': 'Freelance',
            'category.investment': 'Inversión',
            'category.gift': 'Regalos',
            'settings.language': 'Idioma',
            'settings.currency': 'Moneda',
            'settings.theme': 'Tema',
        },

        me: {
            appName: 'FinFlow',
            tagline: 'Pametno upravljanje finansijama',
            addTransaction: 'Dodaj transakciju',
            newTransaction: 'Nova transakcija',
            save: 'Sačuvaj',
            cancel: 'Otkaži',
            delete: 'Obriši',
            reset: 'Resetuj',
            balance: 'Balans',
            income: 'Prihodi',
            expense: 'Rashodi',
            transactions: 'Transakcije',
            amount: 'Iznos',
            category: 'Kategorija',
            description: 'Opis',
            date: 'Datum',
            type: 'Tip',
            filters: 'Filteri',
            allTypes: 'Svi tipovi',
            allCategories: 'Sve kategorije',
            from: 'Od',
            to: 'Do',
            noTransactions: 'Nema transakcija',
            addFirst: 'Dodajte prvu transakciju',
            confirmDelete: 'Obrisati transakciju?',
            noData: 'Nema podataka',
            required: 'obavezno',
            amountRequired: 'Unesite iznos',
            categoryRequired: 'Odaberite kategoriju',
            dateRequired: 'Odaberite datum',
            invalidAmount: 'Unesite validan iznos',
            amountMustBePositive: 'Iznos mora biti pozitivan',
            descriptionPlaceholder: 'Npr: Kupovina namirnica',
            dashboard: 'Početna',
            analytics: 'Analitika',
            analyticsDescription: 'Vizualizacija i analiza vaših finansija',
            week: 'Nedjelja',
            month: 'Mjesec',
            year: 'Godina',
            totalIncome: 'Ukupni prihodi',
            totalExpenses: 'Ukupni rashodi',
            savingsRate: 'Stopa štednje',
            avgDailyExpense: 'Prosječan dnevni rashod',
            topCategory: 'Glavna kategorija',
            noExpenseData: 'Nema podataka o rashodima',
            noTransactionData: 'Nema podataka o transakcijama',
            expensesByCategory: 'Rashodi po kategorijama',
            incomeExpenseTrend: 'Trend prihoda i rashoda',

            // NEW ANALYTICS TRANSLATIONS
            financialAnalytics: 'Finansijska analitika',
            selectPeriod: 'Odaberite period',
            lastWeek: 'Prošle nedjelje',
            lastMonth: 'Prošlog mjeseca',
            lastYear: 'Prošle godine',
            days: 'dana',
            netBalance: 'Neto balans',
            allIncomingTransactions: 'Sve dolazne transakcije',
            allExpenseTransactions: 'Sve transakcije rashoda',
            positiveBalance: 'Pozitivan balans',
            negativeBalance: 'Negativan balans',
            ofIncomeSaved: 'od prihoda ušteđeno',
            topSpendingCategory: 'Glavna kategorija rashoda',
            highestExpenseCategory: 'Najveća kategorija rashoda',
            transactionCount: 'Broj transakcija',
            totalTransactionsInPeriod: 'Ukupno transakcija u periodu',
            averageTransaction: 'Prosječna transakcija',
            perTransaction: 'po transakciji',
            expenseDistribution: 'Distribucija rashoda',
            incomeVsExpenses: 'Prihodi vs Rashodi',

            topCategories: 'Top kategorije',
            balanceTrend: 'Trend balansa',
            incomeVsExpense: 'Prihodi vs Rashodi',
            totalTransactions: 'Ukupno transakcija',
            'category.food': 'Hrana i namirnice',
            'category.transport': 'Prevoz',
            'category.housing': 'Stanovanje',
            'category.entertainment': 'Zabava',
            'category.health': 'Zdravlje',
            'category.education': 'Obrazovanje',
            'category.clothing': 'Odjeća',
            'category.subscriptions': 'Pretplate',
            'category.other': 'Ostalo',
            'category.salary': 'Plata',
            'category.freelance': 'Freelance',
            'category.investment': 'Investicije',
            'category.gift': 'Pokloni',
            'settings.language': 'Jezik',
            'settings.currency': 'Valuta',
            'settings.theme': 'Tema',
        },
    };

    export type TranslationKey = keyof typeof translations.ru;